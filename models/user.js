    ...
    User.findOne({ username: username })
        .exec(function (err, user) {
            if (err) {
                ...
                var err = new Error('User login already exists.');
                err.status = 401;
                return callback(err);
            } else {
               return callback();
...
UserSchema.statics.createNewAccount = function (login, pass, callback) {
    var user = new User({username: login, password: pass});
...
UserSchema.statics.getSimularLogin = function (part, callback) {
    User.find({username: {$regex : ".*" + part + ".*"}}).exec(function (err, data) {
        if(err) {
            return callback(err);
        } else {
